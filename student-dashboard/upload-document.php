<?php
  ...
  $sel = $conn->query("SELECT * FROM document WHERE id = '$document_id'");
  while($row = $sel->fetch_assoc()) {
    $document_name = $row['name'];
  }

  $sel2 = $conn->query("SELECT * FROM student WHERE matric_no = '$student_id'");
  while($rows = $sel2->fetch_assoc()) {
  ...
  if(isset($_POST['uploadBtn'])) {
    ...
    $document_file = $_FILES['document_file']['name'];
		$directory = "../documents/".$document_file;
		$target_file = $directory.basename($document_file);
		move_uploaded_file($_FILES['document_file']['tmp_name'], $directory);

    $insert = $conn->query("INSERT INTO upload_document(document_id, matric_no, document_name, document_file, student_name, `status`) VALUES('$document_id', '$student_id', '$document_name', '$document_file', '$student_name', 'pending')");
